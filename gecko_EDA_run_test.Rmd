...
```{r}
#read data
gecko_train_mean_sd <- read_csv(file.path(here(), "data", "gecko_train_mean_sd.csv"))
gecko_train_mean_sd <- gecko_train_mean_sd[,-1]
gecko_full_vars <- read_csv(file.path(here(), "data", "test.csv"))
gecko_full_vars$pSat_Pa <- 1 #dummy
```
...
Scaling test for full data

```{r}
#train mean
gecko.train.mean <- gecko_train_mean_sd[,-1] |> lapply(function(x)mean(x, na.rm = TRUE))
#train sd
gecko.train.sd <- gecko_train_mean_sd[,-1] |> lapply(function(x)sd(x, na.rm = TRUE))
# scale all columns except id using train mean and sd
gecko_num_scaled <- (gecko_num[,-1] - gecko.train.mean)/gecko.train.sd
```
...
gecko_ml_test <- 
  gecko_ml |> 
  select(-contains("pSat_Pa"))

write_csv(gecko_ml_test, "data/gecko_ml_test.csv")
```
